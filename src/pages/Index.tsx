...
import { Ghost, MoonStar, ScrollText, Skull, Sparkles } from 'lucide-react';

const featureHighlights = [
  {
    title: 'Enchanted Whispers',
    description: 'Pen candlelit missives that flicker to life only when the proper incantation is spoken aloud.',
    icon: <MoonStar className="h-6 w-6 text-orange-300" />,
  },
  {
    title: 'Coven-Crafted Security',
    description: 'Trade rote seed phrases for clever riddles so only sharp-minded allies can lift your protective charm.',
    icon: <Skull className="h-6 w-6 text-purple-200" />,
  },
  {
    title: 'Haunted Relays',
    description: 'Let your bewitched notes haunt every Nostr relay you trust, carrying your puzzles across the ether.',
    icon: <Ghost className="h-6 w-6 text-sky-200" />,
  },
];
...
const Index = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-midnight-blend text-slate-100 bg-grain">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-fogged" />
          ...
          <div className="space-y-10">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <CreateHexedNoteDialog
                trigger={
                  <Button size="lg" className="inline-flex h-12 items-center rounded-full bg-gradient-to-r from-purple-500 via-indigo-500 to-sky-400 px-8 text-base font-medium text-slate-50 shadow-xl shadow-purple-900/30">
                    <SpellIcon className="h-5 w-5 text-slate-50" />{' '}
                    Cast Hex
                  </Button>
                }
              />
              <Button
                ...
                className="inline-flex h-12 items-center rounded-full border-slate-700 bg-slate-900/70 px-8 text-base text-slate-200 backdrop-blur transition hover:border-slate-500"
              >
                <Ghost className="h-5 w-5 text-slate-200" />{' '}
                Explore Hexes
              </Button>
            </div>
          ...
          <div className="relative">
            ...
            <div className="relative space-y-6 rounded-3xl border border-white/10 bg-slate-900/60 p-8 backdrop-blur-xl">
              <header className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-display text-sm uppercase tracking-[0.2em] text-purple-200">Demo Hex</p>
                  <p className="font-display text-lg font-semibold">“Riddle: speak friend and enter.”</p>
                </div>
              </header>
              ...
              <div className="space-y-4 rounded-2xl border border-purple-500/40 bg-slate-950/60 p-6 shadow-[0_0_40px_-20px] shadow-purple-500/60">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Hexed Message</p>
          ...
          {featureHighlights.map((feature) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-900/80">
                  {feature.icon}
                </div>
                <h2 className="font-display text-xl font-semibold text-slate-50">{feature.title}</h2>
              </div>
              <p className="mt-6 text-sm text-slate-300">{feature.description}</p>
            </div>
          ))}
        </section>
      </div>
